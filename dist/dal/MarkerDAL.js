...
export class MarkerDAL {
    static async selectById(id) {
        try {
            ...
            if (res.rowCount > 0) {
                return new MarkerModel(res.rows[0].user_id, res.rows[0].title, res.rows[0].id);
            }
            return null;
        }
        catch (error) {
    ...
    static async selectByUserId(userId) {
        try {
            ...
            const res = await DB.pool.query(query);
            if (res.rowCount > 0) {
                return res.rows.map(row => new MarkerModel(row.user_id, row.description, row.id));
            }
            return [];
        }
        catch (error) {
    ...
    static async selectByNoteId(noteId) {
        try {
            ...
            const res = await DB.pool.query(query);
            if (res.rowCount > 0) {
                return res.rows.map(row => new MarkerModel(row.user_id, row.description, row.marker_id));
            }
            return [];
        }
        catch (error) {
    ...
    static async create(marker) {
        try {
            ...
            const res = await DB.pool.query(query);
            if (res.rowCount > 0) {
                return new MarkerModel(res.rows[0].user_id, res.rows[0].description, res.rows[0].id);
            }
            return null;
        }
        catch (error) {
