...
export class UserDAL {
    static async select() {
        try {
            ...
            if (res.rowCount > 0) {
                return res.rows.map(row => new UserModel(row.nome, row.email, row.senha, row.id, row.foto));
            }
            return null;
        }
        catch (error) {
    ...
    static async selectById(id) {
        try {
            ...
            const res = await DB.pool.query(query);
            if (res.rowCount > 0) {
                return new UserModel(res.rows[0].nome, res.rows[0].email, res.rows[0].senha, res.rows[0].id, res.rows[0].foto);
            }
            return null;
        }
        catch (error) {
