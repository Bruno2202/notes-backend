...
class TokenController {
    static async generateToken(request, reply) {
        try {
            ...
            const token = TokenService_js_1.TokenService.generateToken(id, email);
            if (token) {
    ...
    static getTokenData(request, reply) {
        try {
            ...
            const isValid = TokenService_js_1.TokenService.validateToken(token);
            if (isValid) {
                const data = TokenService_js_1.TokenService.getTokenData(token);
                if (data) {
